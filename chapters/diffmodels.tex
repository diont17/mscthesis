...
Generally, the two models predict the same behaviour at short echo times \SIrange{1}{3}{ms} and at long echo times (\SI{>20}{ms}) \cite{BrooksT2shorteningweaklymagnetized2001}, but in the intermediate range, the two models give different values for the decrease in \Ttwo as echo time changes.
...
In their paper, Jensen and Chandra study this effect using the weak-field approximation \cite{JensenNMRrelaxationtissues2000}.
...
In all fits, the diffusion coefficient of water was assumed to be \SI[per-mode=reciprocal]{2.0}{\micro\metre\squared\per\milli\second}.
...
CPMG decays were measured for blood in the continuous flow setup at each of the field strengths used in those experiments.
...
1.75 is still close to the 2 predicted by theory \cite[Eq. 52-54]{JensenNMRrelaxationtissues2000}.
...
As both models have acceptable predictions for the shape of the echo time curve, these results cannot provide more evidence of the actual physical mechanism of the \Ttwo shortening effect.
