...
As in the stopped flow experiments, signal before \SI{15}{ms} was ignored in the fitting process to remove the signal from the tube.
...
In particular, the outflow effect becomes significant as the time in the coil (\SI{2}{cm} long) becomes shorter than the experiment time, as the decrease in spins from the coil adds to the true \Ttwo decay and causes the \Ttwo to appear shorter.
...
There were a number of possible causes for this effect, including absolute phase effects from the timing in the Kea spectrometer and changing sources of RF noise.
...
The flow circuit was modified to include another blood bag between the magnet and the pump, so the blood flowed under gravity.
...
After the initial temperature stabilising section, the pH was \num{7.32} and haematocrit was \num{0.35}.
...
The initial \Ttwo is around \SI{350}{ms}, which is quite high compared to the stopped flow results (Spinsolve at 42 MHz measured \SI{310}{ms} for initial \Ttwo.
...
The NMR experiment was switched off around minute 280, and there appear to be artefacts at minute 293 and 300.
...
Because of the artefacts in ramp up 1, ramp down 2 and ramp up 2, only the first deoxygenation ramp has been used for further analysis.
...
There is still a significant drop as the blood is deoxygenated, and the splitting between different echo times is still clear.
...
Additionally, the same splitting occurs between the short and long echo times as the first oxygenated stage.
...
This oscillation was not observed in the other samples, and the size of the temperature dependence is larger than would be expected from other experiments (see 5 MHz data).
...
Where data was taken from multiple oxygenation ramps, the \Ttwo values were shifted up, so that \Ttwo of oxygenated blood matched the initial measurements.
...
The fit results show that there is a significant linear correlation in the parameters at most of the different field strengths.
...
The temperature of the blood in the circuit was controlled using flow from a \SI{37}{\celsius} water bath through the oxygenator, although because of variations in the blood flow rate through the oxygenator, the mixing process which occured in the blood bags, and changes in the room temperature, in most cases the temperature was only stable within a \num{3}-\SI{4}{\celsius} range.
...
In blood, the haematocrit is also known to affect the \Ttwo changes.
...
In contrast the continuous flow results show a similar trend, but with \textit{a}=\SI{0.11\pm0.12}{s^{-1}} and \textit{b}=\SI{5.1\pm0.3}{s^{-1}/T^2}.
...
This extrapolated value would be expected to be lower, as these continuous flow experiments used blood samples with a lower haematocrit than the literature studies, which used blood with a physiological \Hct, and a slightly shorter echo time (\SI{5}{ms}), rather than the \SI{6}{ms} in studies by Wright).
...
It is possible that haemolysis is occurring due to the blood flowing through the relatively tight screw clamp, or due to contact with the PVC tube and oxygenator membrane.
...
More recently, a parametric method of fitting \Ttwo values measured at multiple echo times was introduced by Varghese  for use in MRI oximetry of the heart \cite{VargheseCMRbasedbloodoximetry2017}.
...
\centering
\makebox[\textwidth][c]{\includegraphics[width=\textwidth]{figures/contflow/calibratedNMRSO2.pdf}}
...
The size of the \Ttwo change is proportional  on \Bzero
